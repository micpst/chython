...
# 8 bit - 0x04 (current format specification)
# 12 bit - number of atoms
# 12 bit - cis/trans stereo block size
#
# Atom block 10 bytes (repeated):
# 12 bit - atom number
...
def unpack(const unsigned char[::1] data not None):
    cdef char *charges
    cdef unsigned char a, b, c, d, isotope, neighbors_count, s = 0, nc, version
    cdef unsigned char *hydrogens, *neighbors, *orders, *is_tet, *is_all
    cdef bint *stereo_sign, *ct_sign, *radicals
    cdef unsigned short atoms_count, atomic_number, bonds_count = 0, cis_trans_count, order_count
    cdef unsigned short i, j, k = 0, n, m, buffer_b, shift = 0
    cdef unsigned short *atoms, *mapping, *isotopes, *cis_trans_1, *cis_trans_2, *connections
    ...
    atoms = <unsigned short*> PyMem_Malloc(atoms_count * sizeof(unsigned short))
    ...
    for i in range(atoms_count):
        ...
        a, b = data[atoms_shift + 5], data[atoms_shift + 6]
        x_coord[i] = double_from_bytes(a, b)
        a, b = data[atoms_shift + 7], data[atoms_shift + 8]
        y_coord[i] = double_from_bytes(a, b)

        a = data[atoms_shift + 9]
        hydrogens[i] = a >> 5
