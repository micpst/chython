...
def pack(object molecule):
    ...
    cdef char charge
    cdef unsigned char isotope, bond, s = 0, buffer_b, buffer_o
    cdef unsigned char *p, *data
    cdef unsigned short atoms_count, atomic_number, bonds_count = 0, cis_trans_count, n, m
    ...
    if size % 8:  # partial byte fill
        ...
        size /= 8
    bonds_shift = 4 + 10 * atoms_count  # connection table starting byte
    ...
    data[0] = 4  # header. specification version 4
    ...
    for py_obj, py_atom in py_atoms.items():
        ...
        data[atoms_shift + 3] = isotope << 7 | atomic_number >> 8 # 1bI , A
        data[atoms_shift + 4] = atomic_number & 0xFF
        data[atoms_shift + 5] = p[0]
        data[atoms_shift + 6] = p[1]
        data[atoms_shift + 7] = p[2]
        data[atoms_shift + 8] = p[3]
        data[atoms_shift + 9] = hcr[n]
        atoms_shift += 10
